        ...
        <div class="w-full ">
            <table class="w-full border-separate border-spacing-y-4">
                ...
                <tr v-for="el in getList" :key="el.id" class="shadow text-center">
                    <td class="flex items-center px-3 py-3 "> <img :src="el.thumbnail" class="w-[50px] h-[39px]" alt=""> {{el.title}}</td>
                    <td> {{el.price}}$</td>
                    <td> <input type="number" :id="el.id" @input="updateTotal(el)" value="1" min="0" placeholder="1" class="w-[50px] text-center border"> </td>
                    <td> {{ el.price }}$ </td>
                </tr>
            </table>  
            ...
            <div class="flex justify-between mb-[140px]">
                <div>
                    <input type="text" class="h-[56px] w-[300px] border rounded-md pl-4" placeholder="Coupon code">
                    <button class="px-6 py-4 rounded-md ml-[20px] bg-[#DB4444]">Apply coupon</button>
                </div>
                <div class="w-[470px] h-[324px] border-2 border-black rounded-md px-[24px] py-[32px] flex flex-col gap-3">
                    ...
                    <router-link to="/checkout" class="mx-auto"><button class="px-5 py-3 rounded-md ml-[20px] my-[20px] text-white bg-[#DB4444]">Process to checkout</button></router-link>
                </div>
            </div>            
        </div>
    </div>
